-- demo-data.sql — Reference lookups, demo persons and notes.
--
-- Run after demo-schema.sql. Wipes the relevant tables and reseeds
-- them with a curated demo dataset:
--   * countries, cities, roles, departments, employment types,
--     notify channels, languages
--   * 10 persons with realistic spread of nationalities, roles, ages,
--     net worths and notification preferences
--   * 12 notes attached to those persons
--
-- TRUNCATE ... CASCADE is used so the script is idempotent — running
-- it again starts from a clean slate. RESTART IDENTITY resets the
-- serial counters so person ids stay 1..10, note ids 1..12 across
-- reseeds (the demo notes reference person ids by literal value).
