...
/// Builds an owned CString for an optional quantization mode. The value
/// must outlive the FFI call so the pointer stays valid; `None` selects
/// the affine default.
fn mode_cstring(mode: Option<&str>) -> Option<std::ffi::CString> {
    mode.map(|m| std::ffi::CString::new(m).expect("invalid quantization mode string"))
}

/// Resolves an optional mode CString to the C pointer the MLX quant ops
/// expect, falling back to the affine default when no mode is given.
fn mode_ptr(mode: &Option<std::ffi::CString>) -> *const std::os::raw::c_char {
    mode.as_ref()
        .map(|c| c.as_ptr())
        .unwrap_or(DEFAULT_MODE.as_ptr())
}
